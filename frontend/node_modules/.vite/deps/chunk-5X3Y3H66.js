...
import {
  ...
  WindowContext,
  useLocaleText
} from "./chunk-Z7GN3LZG.js";
import {
  Alert_default
} from "./chunk-XHYLBO5X.js";
import {
  Button_default,
  IconButton_default
...
function DialogsProvider(props) {
  ...
  const keyPrefix = React10.useId();
  const nextId2 = React10.useRef(0);
...
function useStorageState(area, key, initializer = null, options) {
  ...
  const [initialValue] = React11.useState(initializer);
  const encodedInitialValue = React11.useMemo(() => encode(codec, initialValue), [codec, initialValue]);
  const subscribeKey = React11.useCallback((callback) => subscribe(area, key, callback), [area, key]);
  const getKeySnapshot = React11.useCallback(() => getSnapshot(area, key) ?? encodedInitialValue, [area, encodedInitialValue, key]);
  const encodedStoredValue = React11.useSyncExternalStore(subscribeKey, getKeySnapshot, getKeyServerSnapshot);
  const storedValue = React11.useMemo(() => decode(codec, encodedStoredValue), [codec, encodedStoredValue]);
  const setStoredValue = React11.useCallback((value) => {
  ...
  const [nonStoredValue, setNonStoredValue] = React11.useState(initialValue);
  if (!key) {
...
function LegacyThemeProvider(props) {
  ...
  const dualAwareTheme = React12.useMemo(() => isDualTheme ? theme[paletteMode === "dark" ? "dark" : "light"] ?? theme[paletteMode === "dark" ? "light" : "dark"] : theme, [isDualTheme, paletteMode, theme]);
  const paletteModeContextValue = React12.useMemo(() => ({
    paletteMode,
...
var AuthenticationContext = React13.createContext(null);
var SessionContext = React13.createContext(null);
...
react-is/cjs/react-is.development.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-5X3Y3H66.js.map
