"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
var _exportNames = {
  useMovieData: true,
  loadServerRows: true
};
Object.defineProperty(exports, "loadServerRows", {
  enumerable: true,
  get: function () {
    return _serverUtils.loadServerRows;
  }
});
Object.defineProperty(exports, "useMovieData", {
  enumerable: true,
  get: function () {
    return _useMovieData.useMovieData;
  }
});
var _useDemoData = require("./useDemoData");
Object.keys(_useDemoData).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (Object.prototype.hasOwnProperty.call(_exportNames, key)) return;
  if (key in exports && exports[key] === _useDemoData[key]) return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function () {
      return _useDemoData[key];
    }
  });
});
var _useBasicDemoData = require("./useBasicDemoData");
Object.keys(_useBasicDemoData).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (Object.prototype.hasOwnProperty.call(_exportNames, key)) return;
  if (key in exports && exports[key] === _useBasicDemoData[key]) return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function () {
      return _useBasicDemoData[key];
    }
  });
});
var _useMovieData = require("./useMovieData");
var _useQuery = require("./useQuery");
Object.keys(_useQuery).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (Object.prototype.hasOwnProperty.call(_exportNames, key)) return;
  if (key in exports && exports[key] === _useQuery[key]) return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function () {
      return _useQuery[key];
    }
  });
});
var _useMockServer = require("./useMockServer");
Object.keys(_useMockServer).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  if (Object.prototype.hasOwnProperty.call(_exportNames, key)) return;
  if (key in exports && exports[key] === _useMockServer[key]) return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function () {
      return _useMockServer[key];
    }
  });
});
var _serverUtils = require("./serverUtils");